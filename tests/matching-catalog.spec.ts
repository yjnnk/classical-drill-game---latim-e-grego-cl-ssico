...
test("numerais filtram o tipo e funcionam nas duas direções", async ({
  page,
}) => {
  ...
  const block = page
    .getByRole("article")
    .filter({ has: page.getByRole("heading", { name: "οἱ ἀριθμοί" }) });
  await expect(block.getByRole("group", { name: "Tipo" })).toContainText(
    "cardinal",
  );
...
test("terminologia usa português e restringe distrações ao tema selecionado", async ({
  page,
}) => {
  await openEditor(page);
  await page.getByRole("button", { name: "Terminologia" }).click();
  await page
    .getByRole("button", { name: /Adicionar γραμματικοὶ ὅροι/ })
    .click();
  const block = page
    .getByRole("article")
    .filter({ has: page.getByRole("heading", { name: "γραμματικοὶ ὅροι" }) });
  const topic = block.getByRole("group", { name: "Tema" });
  for (const checkbox of await topic.getByRole("checkbox").all()) {
    if ((await checkbox.getAttribute("value")) !== "casos")
      await checkbox.uncheck();
  }
  await page.getByLabel("Produção assistida").check();
  await page.getByRole("button", { name: "Iniciar rodada" }).click();
  await expect(page.locator(".analysis-prompt")).toHaveText(
    /caso|nominativo|genitivo|dativo|acusativo|ablativo|vocativo/,
  );
  await expect(page.locator(".analysis-prompt")).not.toContainText(
    /case|nominative|genitive|dative|accusative|ablative|vocative/i,
  );
  await expect(
    page.getByRole("group", { name: "Alternativas" }).getByRole("button"),
  ).toHaveCount(3);
...
test("um bloco de correspondência combina com um bloco morfológico", async ({
  page,
}) => {
  await openEditor(page);
  await page.getByRole("button", { name: "Numerais" }).click();
