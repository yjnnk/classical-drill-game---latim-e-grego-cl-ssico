...
test("oferece retomar exatamente a pergunta ativa após recarregar", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "Iniciar rodada" }).first().click();
  const prompt = await page.locator(".prompt > p").nth(1).textContent();

  await page.reload();
  await expect(page.getByText("Rodada em andamento", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: "Iniciar rodada" }).first()).toBeDisabled();
...
test("abandonar remove a rodada sem criar histórico", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Iniciar rodada" }).first()).toBeEnabled();
});
