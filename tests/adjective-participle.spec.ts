...
test("adjetivos são encontráveis e oferecem somente filtros aplicáveis", async ({
  page,
}) => {
  ...
  const block = page
    .getByRole("article")
    .filter({ has: page.getByRole("heading", { name: /καλός/ }) });
...
test("comparações praticam grau sem inventar caso, número ou gênero", async ({
  page,
}) => {
  await page.getByRole("button", { name: "Adjetivos" }).click();
  ...
  const block = page
    .getByRole("article")
    .filter({ has: page.getByRole("heading", { name: "ἀνδρεῖος" }) });
