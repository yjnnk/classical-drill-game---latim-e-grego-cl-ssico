...
test("verbos regulares, contractos, em -μι, depoentes e irregulares são encontráveis", async ({ page }) => {
...
for (const [paradigmId, lemma] of [
  ...
  for (const direction of ["Análise", "Produção assistida"]) {
    test(`${lemma} inicia em ${direction}`, async ({ page }) => {
      const directionValue = direction === "Análise" ? "analysis" : "production";
      await page.goto("/");
      await page.evaluate(({ paradigmId, directionValue, lemma }) => {
        localStorage.clear();
        localStorage.setItem("classical-drill-decks:v1", JSON.stringify({
          version: 1,
          decks: [{
            id: "deck:test",
...
test("formas finitas podem ser recortadas e praticadas em Análise", async ({ page }) => {
  await openEditor(page);
  await page.getByLabel("Pesquisar paradigmas").fill("soltar");
  await page.getByRole("button", { name: "Adicionar λῡ́ω" }).evaluate(
  ...
  await expect(block.getByRole("group", { name: "Tempo" })).toContainText("presente");
  await expect(block.getByRole("group", { name: "Voz" })).toContainText("ativo");
  await expect(block.getByRole("group", { name: "Modo" })).toContainText("indicativo");
  await expect(block.getByRole("group", { name: "Pessoa" })).toContainText("1ª pessoa");
  await expect(block.getByRole("group", { name: "Número" })).toContainText("singular");
...
test("infinitivos funcionam em Produção assistida sem traços artificiais", async ({ page }) => {
  await openEditor(page);
  await page.getByLabel("Pesquisar paradigmas").fill("soltar");
  await page.getByRole("button", { name: "Adicionar λῡ́ω" }).evaluate(
    (button: HTMLButtonElement) => button.click()
...
test("um recorte verbal sem distrações suficientes é sinalizado antes da rodada", async ({ page }) => {
  await openEditor(page);
  await page.getByLabel("Pesquisar paradigmas").fill("soltar");
  await page.getByRole("button", { name: "Adicionar λῡ́ω" }).evaluate(
    (button: HTMLButtonElement) => button.click()
